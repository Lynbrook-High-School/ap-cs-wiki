...
const handleSubmittedAnswerAtom = atom(null, (get, set, answer_index: number) => {
...
const QuizAnswerExplanation = (props: { children?: React.ReactNode }) => {
  return (
    <div className="no-y-margin text-sm text-gray-700 dark:text-gray-400">{props.children}</div>
  )
}
...
const QuizMCAnswer = (props) => {
  ...
  const showVerdict = submitted && (isSelected || correctAnswers.includes(selectedAnswer ?? -1)) //display correctness/explanation
...
const ActualQuiz = (props) => {
  ...
  const questionList: React.ReactElement[] = React.Children.map(props.children, (child) => {
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        >
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className="-ml-0.5 mr-2 size-6 h-4 w-4"
          >
            <path
              strokeLinecap="round"
        ...
        >
          ...
          {canMoveOn && (
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="-mr-0.5 ml-2 size-6 h-4 w-4"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
