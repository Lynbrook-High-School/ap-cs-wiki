...
Well, we know that the while loop can do up to $\mathcal{O}(n)$ loops in a single iteration of $i$. 
But, because `right` can only increase, it can only move up to $\mathcal{O}(n)$ times. Thus, 
the time complexity of this program is $\mathcal{O}(n)$.
